...
if(.%1)==(.) echo %MSBuild% Radegast.sln /p:Configuration=Release >> compile.bat

if(.%1)==(.msbuild) echo echo ==== COMPILE BEGIN ==== >> compile.bat
if(.%1)==(.msbuild) echo %MSBuild% /p:Configuration=Release Radegast.sln >> compile.bat
...
if(.%2)==(.docs) echo %MSBuild% /p:Configuration=Release docs\Radegast.shfbproj >> compile.bat
